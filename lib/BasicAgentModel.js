...
var probOrderArrival = 0.8; //0.8
var probDriverArrival = 0.10; //0.1
...
function addDynamicOrders(){
	...
	var randomHouse = houseareas[Math.floor(Math.random() * houseareas.length)];
	...
	switch(state){
		case IDLE:
			if (assignedOrderCounter<orderCounter && orderCounter != 0){
				...
				//else - keep moving around and wait for orders
				// if(hasArrived){
				// 	var randIdleRow =  Math.floor(Math.random() * Math.floor(60))
				// 	var randIdleCol =  Math.floor(Math.random() * Math.floor(60))
				// 	driver.target.row = randIdleRow
				// 	driver.target.col = randIdleCol
				// }
			}
		break;
		...
		case COLLECTINGFOOD:
			...
			//#TO-DO Add some form of waiting/collection time that is dependent on some distribution
		break;
		case DRIVINGTOHOUSE:
			if(hasArrived){
		...
		case DELIVERINGONFOOT:
			//#TO-DO Add some form of delivery time that is dependent on some distribution
