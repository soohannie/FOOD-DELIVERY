...
var probOrderArrival = 0.4;

var probDriverArrival = 0.2;
...
function addDynamicAgents(){
	...
	if (Math.random()< probDriverArrival){
		var newDriver = {
			...
			"state":"IDLE",
			"deliveryCount":0,
			"pickUpOrDropOff":false, //True when driver is picking up order, and False when driver is dropping off the order
			"order":null
		};
...
function updateDriver(driverIndex){
	...
	var pickUpOrDropOff = driver.pickUpOrDropOff
	var currentOrder = driver.order;
	...
	switch(state){
		case IDLE:
			if (assignedOrderCounter!=orderCounter){
				currentOrder = ordersQueue[assignedOrderCounter]
